...
const SignalCrypt = require('./signal-crypt')
...
class Portal {
  constructor ({id, hostPeerId, siteId, peerPool, connectionTimeout, tetherDisconnectWindow}) {
    ...
    // TODO: hostPeerId is only set for join-Portals,
    // for host portals it is set inside peerPool. why? no idea..
    this.signalCrypt = new SignalCrypt(this.id, this.hostPeerId)
    this.peerPool.setSignalCrypt(this.signalCrypt)
