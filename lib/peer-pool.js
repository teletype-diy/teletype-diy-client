...
class PeerPool {
  ...
  injectExternalSignal(encryptedCompressedSignal) {
    // decrypt
    const compressedSignal = this.signalCrypt.decryptRaw(encryptedCompressedSignal);
    console.log(compressedSignal);
    const messageList = zlib.brotliDecompressSync(compressedSignal).toString('utf8');
