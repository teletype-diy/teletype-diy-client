...
module.exports =
class PeerPool {
  constructor ({peerId, peerIdentity, restGateway, pubSubGateway, signalURL, fragmentSize, connectionTimeout, testEpoch, signalBufferLocal}) {
  ...
  async fetchICEServers () {
    ...
    // TODO: clean this up
    const iceServers = ["what do you want??"]
  ...
  injectExternalSignal(messageList) {
    messageList.split("|").forEach((item, i) => {
        if (item) {
            const message = this.signalCrypt.decrypt(item)
