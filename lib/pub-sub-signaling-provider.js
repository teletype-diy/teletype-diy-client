...
class PubSubSignalingProvider {
  constructor ({localPeerId, remotePeerId, restGateway, signalURL, testEpoch}) {
  ...
  async send (signal) {
    ...
    // do not have access to "identity" here right now.
    // it is always me who connects, obviously ...
    if (request.sequenceNumber === 0) request.senderIdentity = "schadomi7"

    // console.log(message);
    // console.log(`/peers/${this.remotePeerId} signal ${message}`);
    ...
    await this.socket.emit("halo", channelName, eventName, data)
