...
class PubSubSignalingProvider {
  constructor ({localPeerId, remotePeerId, signalURL, signalCrypt, username, testEpoch, signalBufferLocal}) {
    ...
    this.signalBufferLocal.setSignalCrypt(this.signalCrypt)
  }
  ...
  async send (signal) {
    ...
    this.signalBufferLocal.interceptSignal(JSON.stringify(request));
    // if (Config.p2pSignalEncrypt()) {
    //     // TODO: I think, this json.stringify is useless, as it is already a string..
    //     // this.signalBufferLocal.interceptSignal(JSON.stringify(data));
    //     this.signalBufferLocal.interceptSignal(data);
    // } else {
    //     this.signalBufferLocal.interceptSignal(JSON.stringify(request));
    // }
